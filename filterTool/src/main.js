...
function updateViewportText(){
	...
	var x1=receiveX+scaleWidth+100*(canvas.width/min_width);
	var x2=receiveX+scaleWidth+100*(canvas.width/min_width)-temp-100;
	ctx2.clearRect(x1,0,temp,canvas.height/4);
	ctx2.clearRect(x2,0,temp,canvas.height/4);
...
function draw2DView(){
	...
	if(mapCIndices[0]<scales.length){
		...
		//ipanel.move(200,0,0);
		//ipanel.scale(img_data[imgIndex].w,img_data[imgIndex].h);
		//ipanel.draw();
...
var ImagePanel=function(x,y,w,h,dataID,cID){
	...
	this.drawInViewport=function(vID,inverted){
		...
		//At this point, the texture stored is colored and has an image
		//We need to hijack it and test our fft on it
...
function start() {
	//Let's start by testing out this new matrix library
...
function initViewport(){ 
	...
	screenshot1.style.left=(x1-35*screenscale)+"px";
	screenshot2.style.top=(y+temp+20)+"px";
	screenshot2.style.left=(x2-35*screenscale)+"px";
	
	var filter1=document.getElementById("filter1");
	var filter2=document.getElementById("filter2");
	filter1.style.top=(y+temp-50)+"px";
	filter1.style.left=(x1-35*screenscale)+"px";
	filter2.style.top=(y+temp-50)+"px";
	filter2.style.left=(x2-35*screenscale)+"px";
	
	var invertbutton1=document.getElementById("invert1");
	var invertbutton2=document.getElementById("invert2");
	
	invertbutton1.style.top=(y+50)+"px";
	invertbutton1.style.left=(x1-35*screenscale)+"px";
	invertbutton2.style.top=(y+50)+"px";
	invertbutton2.style.left=(x2-35*screenscale)+"px";
	
	var result1 = document.getElementById("Result1");
	result1.style.top=(y+temp+20)+"px";
	result1.style.left=x2+"px";
	
	var result2 = document.getElementById("Result2");
	result2.style.top=(y+temp+20)+"px";
	result2.style.left=x1+"px";
	
	var cutoff1 = document.getElementById("cutoff1");
	cutoff1.style.top=(y+temp+70)+"px";
	cutoff1.style.left=(x2-45)+"px";
	cutoff1.style.width=40+"px";
	
	var cutoff2 = document.getElementById("cutoff2");
	cutoff2.style.top=(y+temp+70)+"px";
	cutoff2.style.left=(x1-45)+"px";
	cutoff2.style.width=40+"px";
	
	var pass1 = document.getElementById("pass1");
	pass1.style.top=(y+temp+100)+"px";
	pass1.style.left=(x1-45)+"px";
...
function testreceiverHit(mouseX,mouseY){
	//Test y valuesasd
	for(var i=0;i<mapCIndices.length;i++){
		var temp = viewports[i];
		if(mouseX>temp.x-iconWidth/2&&mouseX<temp.x+temp.h+iconWidth/2&&mouseY>temp.y-iconHeight/2&&mouseY<temp.y+temp.h+iconHeight/2){
			return i;
		}
	}
	return -1;
...
function FileListenerInit(){
	...
	if(window.FileReader) {
		addEventHandler(window, 'load', function() {
			...
			addEventHandler(filter1,'click', function(){updateFilterView(0);});
			addEventHandler(filter2,'click', function(){updateFilterView(1);});
...
function handlePassButton(id){
	var button = document.getElementById("pass"+(id+1));
	if(highPass[id]===false){
		button.src="../icon/high.png";
		highPass[id]=true;
	}
	else{
		button.src="../icon/low.png";
		highPass[id]=false;
	}
	updateFilterView(id);
}
...
function updateFilterView(id){
	...
	testFilter(targ.toDataURL("image/png"),id);
}
...
function downloadView(id){
	...
	if(id===1)
		a.href = document.getElementById("Result1").toDataURL();
	else
		a.href = document.getElementById("Result2").toDataURL();
